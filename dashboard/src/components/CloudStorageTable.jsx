...
import { cloudStorageService } from "../services/api" // Import the API service
...
export function CloudStorageTable() {
    ...
    useEffect(() => {
        async function fetchStorageStats() {
            try {
                setLoading(true)
                const response = await cloudStorageService.getConnections()
                console.log(response.data.data.connections)
                setStorageAccounts(response.data.data.connections) // Assuming the response structure contains data
                setLoading(false)
            } catch (e) {
                setLoading(false)
                console.error("Failed to fetch storage connections", e)
            }
        }
    ...
    return (
        ...
        //                             <div className="text-xs text-gray-500 dark:text-gray-400">{account.bucket}</div>
        //                         </td>
        ...
        //                                     <span>{bytesToGB(account.storageStats?.used)} GB</span>
        ...
        //                         <td className="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-400">{formatDate(account.lastSync)}</td>
        ...
        //                                         setLoading(true)
        //                                         await cloudStorageService.syncConnection(menuOpenId)
        //                                         setRefresh(!Refresh)
        //                                         setMenuOpenId(null)
        //                                     }}>
        ...
        //                                     </DropdownMenuItem> */}
        //                                     <DropdownMenuItem onClick={async () => {
        //                                         if (window.confirm('Are you sure you want to disconnect this?')) {
        //                                             setLoading(true)
        //                                             await cloudStorageService.deleteConnection(menuOpenId);
        //                                             setRefresh(!Refresh)
        //                                             setMenuOpenId(null)
        //                                         }
        //                                     }}
