...
export function CollectionsTable() {
    ...
    useEffect(() => {
        async function fetchStorageStats() {
            try {
                setLoading(true)
                const response = await collectionService.getCollections({ type: "storage" })
                console.log(response.data.data.connections)
                setStorageAccounts(response.data.data.collections) // Assuming the response structure contains data
                setLoading(false)
            } catch (e) {
                setLoading(false)
  ...
  <table className="min-w-full divide-y divide-gray-200 border border-gray-300 rounded dark:border-gray-600 my-4">
    <thead className="bg-gray-50 dark:bg-gray-800">
      <tr>
        ...
        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">Prefix</th>
        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">Number of URLs</th>
      ...
      <tbody className="bg-white divide-y divide-gray-200 dark:divide-slate-600 dark:bg-gray-900">
        {storageAccounts.map((account) => (
          <tr className="group cursor-pointer" key={account.id}>
            ...
            <td className="px-6 py-4 whitespace-nowrap">
              <div className="text-xs text-gray-500 dark:text-gray-400">{account.pathPrefix}</div>
            </td>
            <td className="px-6 py-4 whitespace-nowrap">
              <div className="text-xs text-gray-500 dark:text-gray-400">{account.virtualUrls.length}</div>
            </td>
                ...
                <DropdownMenuContent open={menuOpenId === account._id} onClose={() => setMenuOpenId(null)} align="end">
                  <DropdownMenuItem onClick={async () => {
                    if (window.confirm('Are you sure you want to delete this collection?')) {
                      setLoading(true);
                      await collectionService.deleteCollection(menuOpenId);
                      setRefresh(!Refresh);
                      setMenuOpenId(null);
                    }
